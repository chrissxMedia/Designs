(defun svg-attr (key value) (format nil "~d=\"~d\"" key value))
(defun svg-attrs (attrs) (reduce (lambda (x y) (concatenate 'string x " " y)) (map 'list (lambda (x) (svg-attr (car x) (cdr x))) attrs)))
(defun svg-text (x y font size color text)
  (format nil "<text ~d>~d</text>" (svg-attrs (list (cons "x" x) (cons "y" y) (cons "font-family" font) (cons "font-size" size) (cons "fill" color))) text))
(defun svg-rect (&rest attrs)
  (format nil "<rect ~d />" (svg-attrs attrs)))
...
(defun background (color)
  (svg-rect (cons "width" "100%") (cons "height" "100%") (cons "fill" color)))
